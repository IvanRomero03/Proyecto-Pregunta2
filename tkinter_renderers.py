...
def game_upt(win, pgrs):
    ...
    preglbl = tk.Label(win, text=dict_src[0], font=('Helvetica bold', 30), bg=bgcol)
    ...
    for i in range(4):
        if "*" in dict_src[i]:
            cbut = Respuesta(win, True)
            dict_src[i] = dict_src[i].replace("*", " ").strip()
            cbut.button.config(text=dict_src[i], font=('Helvetica bold', 20), bg=btbgcol,
                               command=lambda: cbut.do_check(win, pgrs, but_next))
            cbut.button.grid(column=0, row=i + 1, columnspan=3)
        else:
            vbut = Respuesta(win, False)
            vbut.button.config(text=dict_src[i], font=('Helvetica bold', 20), bg=btbgcol,
                               command=lambda: vbut.do_check(win, pgrs, but_next))
